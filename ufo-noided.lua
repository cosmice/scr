...
local char,mouse = getchar(),funcs.lplr:GetMouse()
local uip=game:GetService("UserInputService")
local hrp=char:WaitForChild("HumanoidRootPart",10)
local hum=char:WaitForChild("Humanoid",10)
local camera = game:GetService("Workspace").CurrentCamera
local UFO = char:WaitForChild("UFO",10)
local ufoh=UFO:FindFirstChild("Handle") or UFO:FindFirstChildOfClass("Part")
local fly = false
...
o109.Position = Vector3.new(12.0515394, 40.7946739, -70.0561295)
o1.PrimaryPart = o109
o1:PivotTo(CFrame.new(ufoh.CFrame.Position))
if PlayAudios then
...
local function jrequest()
while issitting and hrp and seat do
hrp.CFrame=seat.CFrame * CFrame.new(0, seat.Size.Y/2+1.32495713, seat.Size.Z / 2)
task.wait()
end
end
--[[local seatweld=Instance.new("Motor6D")
seatweld.Name="SeatWeld"
seatweld.Part0=seat
seatweld.C0=CFrame.new(0, .5, 0, 1, 0, -0, 0, 0, 1, 0, -1, 0)
seatweld.C1=CFrame.new(0, -1.32495713, 0, 1, 0, -0, 0, 0, 1, 0, -1, 0)
seatweld.Parent=seat--]]
local lastsat=-4
task.delay(5,function()
seat.Touched:Connect(function(x)
if x.Parent==char and os.clock()-lastsat>=3 and not issitting and not hum.SeatPart then
issitting=true
lastsat=os.clock()
hum.Sit=true
...
local gamemodule= isfolder("ufoscript") and isfile("ufoscript\\"..game.PlaceId..".lua") and loadfile("ufoscript\\"..game.PlaceId..".lua")()
gmp=nil
			...
			if math.floor(frms/5) == frms/5 then
				coroutine.wrap(function()
					...
					if PlayAudios then
    					...
    					s.Volume = 1
    					s:Play()
						funcs.deb:AddItem(s,2)
    				end
					local pm=part and part:FindFirstAncestorOfClass("Model")
					local ph= pm and pm:FindFirstChildOfClass("Humanoid")
					if part and gamemodule and gamemodule.dmg then
					gamemodule.dmg(ph,50,part,pm,hum,hrp,char)
					end
					funcs.deb:AddItem(trace,.05)
				end)()
			end
			frms = frms + 1
		end)
...
getgenv().ufo_lastufop=ufoh and ufoh.CFrame
...
coroutine.resume(coroutine.create(function()
	while task.wait() do
		for i=0,255,10 do g = i task.wait() end
		for i=255,0,-10 do r = i task.wait() end
		for i=0,255,10 do b = i task.wait() end
		for i=255,0,-10 do g = i task.wait() end
		for i=0,255,10 do r = i task.wait() end
		for i=255,0,-10 do b = i task.wait() end
	end
end))
...
ti(cons, game:GetService("RunService").Heartbeat:Connect(function()
    ufoh.Velocity = Vector3.new(0,-30,0)
end))

ti(cons,uip.InputBegan:Connect(function(x,g)
	if x.UserInputType==Enum.UserInputType.Keyboard and not g then
	if uip:IsKeyDown(Enum.KeyCode.LeftAlt) then
	if x.KeyCode==Enum.KeyCode.R and hrp then
	hrp.CFrame=ufoh.CFrame
	elseif x.KeyCode==Enum.KeyCode.V and hrp then
	hrp.CFrame=ufo_lastufop
	elseif x.KeyCode==Enum.KeyCode.L and hum then
...
hum.Died:Connect(function()
	if ufoh then
	getgenv().ufo_lastufop=ufoh.CFrame
	end   
   for i,v in pairs(cons) do
        v:Disconnect()
    end
	if gamemodule and gamemodule.specialend then
	gamemodule.specialend(char,hum)
	end
	settings()['Physics'].AllowSleep=oldsettings[1]
	settings()['Physics'].PhysicsEnvironmentalThrottle=oldsettings[2]
end)
ufoh:BreakJoints()
align(ufoh, o2, Vector3.new(-1,0,0))
if gamemodule and gamemodule.specialstartfunc then
gamemodule.specialstartfunc(char,hum,hrp)
end
