local hidden = {}
local vrs={} vrs.hiddendeath=Instance.new('Folder') vrs.prnil=function(x,plr) hidden[plr.UserId].oldpar=x.Parent x.Parent=nil local cccon;cccon=x.AncestryChanged:Connect(function() if hidden[plr.UserId] then task.defer(vrs.hiddendeath,x,plr) else cccon=cccon:Disconnect() end end) end
